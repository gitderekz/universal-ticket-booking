// figma-frontend/src/app/pages/Staff/RoutesManagement.tsx
import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { useTranslation } from 'react-i18next';
import { useCurrency } from '../../../contexts/CurrencyContext';
import { getRoutes, createRoute, updateRoute, deleteRoute } from '../../../services/managementService';
import { getTransports } from '../../../services/adminService';
import { Plus, MapPin, Edit, Trash2, Search, AlertTriangle, X, Train, Bus, Plane, Ship } from 'lucide-react';
...
export const RoutesManagement = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [routeData, transportData, stationsRes] = await Promise.all([
          getRoutes(),
          getTransports(1, 100, ''),
          getStations(), // Use the service method
        ]);
        setRoutes(routeData || []);
  ...
  const filteredRoutes = routes.filter(route => {
    const origin = route.originStation?.name || '';
    const destination = route.destinationStation?.name || '';
  ...
  const stats = {
    ...
    totalDistance: routes.reduce((sum, r) => sum + (parseFloat(r.distance_km) || 0), 0),
  };
  ...
  const handleOpenModal = (route?: any) => {
    if (route) {
      ...
      setFormData({
        transportId: route.transport_id,
        ...
        stations: route.RouteStations?.map((rs: any) => ({
          station_id: rs.station_id,
          sequence_order: rs.sequence_order,
          cumulative_price: parseFloat(rs.cumulative_price) || 0,
          is_break_stop: rs.is_break_stop || false,
          stop_duration_minutes: rs.stop_duration_minutes || 0,
        })) || [],
      });
    } else {
  ...
  const addStation = () => {
    setFormData({
      ...
      stations: [
        ...
        {
          station_id: '',
          sequence_order: formData.stations.length + 1,
          cumulative_price: 0,
          is_break_stop: false,
  ...
  const removeStation = (index: number) => {
    const updatedStations = formData.stations.filter((_, i) => i !== index);
    // Reorder sequence numbers
    updatedStations.forEach((station, idx) => {
      station.sequence_order = idx + 1;
    });
    setFormData({ ...formData, stations: updatedStations });
  };
  ...
  const handleSave = async () => {
    ...
    if (!formData.originStationId) newErrors.originStationId = 'Origin station is required';
    if (!formData.destinationStationId) newErrors.destinationStationId = 'Destination station is required';
    ...
    const originStation = stations.find(s => s.id === formData.originStationId);
    const destinationStation = stations.find(s => s.id === formData.destinationStationId);

    try {
      const payload = {
        company_id: transport.company_id,
        transport_id: formData.transportId,
        name: formData.name || `${originStation?.name} → ${destinationStation?.name}`,
        description: formData.description || `${originStation?.name} to ${destinationStation?.name}`,
        origin_station_id: formData.originStationId,
        destination_station_id: formData.destinationStationId,
        base_price: formData.base_price,
        distance_km: formData.distance_km,
        status: formData.status,
        stations: formData.stations.map(s => ({
          station_id: s.station_id,
          sequence_order: s.sequence_order,
          cumulative_price: s.cumulative_price,
      ...
      if (editingRoute) {
        ...
        addLog({
          ...
          action: `Updated route: ${payload.name}`,
          module: 'routes',
          status: 'success',
          details: `Price: ${formatPrice(payload.base_price)}`,
        });
        ...
        addLog({
          userId: user?.id || '',
          userName: user?.fullName || '',
          action: `Created route: ${payload.name}`,
          module: 'routes',
          status: 'success',
          details: `Price: ${formatPrice(payload.base_price)}`,
        });
  ...
  const handleDelete = async (route: any) => {
    try {
      ...
      addLog({
        userId: user?.id || '',
        userName: user?.fullName || '',
        action: `Deleted route: ${route.name}`,
        module: 'routes',
        status: 'warning',
      });
  ...
  const getTransportIcon = (type: string) => {
    if (type === 'aeroplane') return Plane;
    if (type === 'ferry' || type === 'ship') return Ship;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t('nav.routes')}</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage transport routes and station stops</p>
        </div>
        <motion.button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 p-6 rounded-xl text-white shadow-lg">
          <p className="text-sm opacity-90 mb-1">Active Routes</p>
          <p className="text-3xl font-bold">{stats.active}</p>
        </div>
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-6 rounded-xl text-white shadow-lg">
          <p className="text-sm opacity-90 mb-1">Total Distance</p>
          <p className="text-3xl font-bold">{stats.totalDistance.toFixed(0)} km</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <AnimatePresence mode="popLayout">
          {filteredRoutes.map((route, index) => {
            const transport = transports.find(t => t.id === route.transport_id);
            const transportType = transport?.TransportType;
            const Icon = getTransportIcon(transportType?.slug || 'bus');
            const stationsList = route.RouteStations?.map((rs: any) => rs.station).filter(Boolean) || [];
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                        {transport?.name} • {transportType?.name || 'Unknown'}
                      </p>
                      <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                        <span>{stationsList.length} intermediate stops</span>
                        <span>•</span>
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold text-blue-600">{formatPrice(route.base_price)}</p>
                    <p className="text-xs text-gray-500">Full route</p>
                  </div>
                </div>

                {stationsList.length > 0 && (
                  <div className="mb-4">
                    <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Route Stops:</h4>
                    <div className="flex flex-wrap gap-2">
                      <div className="px-3 py-2 bg-green-100 dark:bg-green-900 rounded-lg text-sm">
                        🟢 {route.originStation?.name}
                      </div>
                      {stationsList.map((station: any, idx: number) => (
                        <div key={station.id} className="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg text-sm">
                          📍 {idx + 1}. {station.name}
                        </div>
                      ))}
                      <div className="px-3 py-2 bg-red-100 dark:bg-red-900 rounded-lg text-sm">
      ...
      {/* Add/Edit Modal - Includes station management */}
      <AnimatePresence>
        {showModal && (
          <motion.div
            initial={{ opacity: 0 }}
              ...
              <div className="space-y-4">
                  ...
                  >
                    ...
                    {transports.map(t => (
                      <option key={t.id} value={t.id}>
                        {t.name} - {t.TransportType?.name || 'Unknown'}
                      </option>
                    ))}
                  </select>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    {errors.originStationId && <p className="text-red-500 text-sm mt-1">{errors.originStationId}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.destinationStationId && <p className="text-red-500 text-sm mt-1">{errors.destinationStationId}</p>}
                  </div>
                </div>
                      ...
                      <div key={index} className="flex gap-2 items-start p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        <div className="flex-1 space-y-2">
                          <select
                            ...
                            onChange={(e) => updateStation(index, 'station_id', e.target.value)}
                            className="w-full px-3 py-2 text-sm border rounded-lg"
                          >
                            <option value="">Select Station</option>
                            {stations
                              .filter(s => s.id !== formData.originStationId && s.id !== formData.destinationStationId)
                              .map(s => (
                                <option key={s.id} value={s.id}>{s.name} ({s.city})</option>
                              ))}
                          </select>
                          <div className="flex gap-2">
                            <input
                              type="number"
                              value={station.cumulative_price}
                              onChange={(e) => updateStation(index, 'cumulative_price', parseFloat(e.target.value) || 0)}
                              className="flex-1 px-3 py-2 text-sm border rounded-lg"
                              placeholder="Price to this stop"
                            />
                            <label className="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border">
                              <input
                                type="checkbox"
                                checked={station.is_break_stop}
                                onChange={(e) => updateStation(index, 'is_break_stop', e.target.checked)}
                                className="w-4 h-4"
                              />
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 mb-6">
                Are you sure you want to delete the route <strong>{deleteConfirm.name}</strong>? This will affect all associated timetables and journeys.
              </p>
